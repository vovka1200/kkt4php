...
class GetShortECRStatus extends Command {
    ...
    static public function flags($data) {
        ...
        if ($data & 0b0000000000000001) {
            $f[] = "Рулон операционного журнала";
        }
        if ($data & 0b0000000000000010) {
            $f[] = "Рулон чековой ленты";
        }
        if ($data & 0b0000000000000100) {
            $f[] = "Верхний датчик подкладного документа";
        }
        if ($data & 0b0000000000001000) {
            $f[] = "Нижний датчик подкладного документа";
        }
        if ($data & 0b0000000000010000) {
            $f[] = "Положение десятичной точки 2";
        }
        if ($data & 0b0000000001000000) {
            $f[] = "Оптический датчик операционного журнала";
        }
        if ($data & 0b0000000010000000) {
            $f[] = "Оптический датчик чековой ленты";
        }
        if ($data & 0b0000000100000000) {
            $f[] = "Рычаг термоголовки операционного журнала опущен";
        }
        if ($data & 0b0000001000000000) {
            $f[] = "Рычаг термоголовки чековой ленты опущен";
        }
        if ($data & 0b0000010000000000) {
            $f[] = "Крышка корпуса ККТ поднята";
        }
        if ($data & 0b0000100000000000) {
            $f[] = "Денежный ящик открыт";
        }
        if ($data & 0b0001000000000000) {
            $f[] = "Отказ правого датчика принтера";
        }
        if ($data & 0b0010000000000000) {
            $f[] = "Отказ левого датчика принтера";
        }
        if ($data & 0b1000000000000000) {
            $f[] = "Увеличенная точность количества";
        }
        return $f;
    }
